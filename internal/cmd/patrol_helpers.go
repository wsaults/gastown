...
func findActivePatrol(cfg PatrolConfig) (patrolID, patrolLine string, found bool) {
	...
	if cfg.CheckInProgress {
		cmdList := exec.Command("bd", "--sandbox", "list", "--status=in_progress", "--type=epic")
	...
	cmdOpen := exec.Command("bd", "--sandbox", "list", "--status=open", "--type=epic")
		...
		for _, line := range lines {
			if strings.Contains(line, cfg.PatrolMolName) && !strings.Contains(line, "[template]") {
				...
				if len(parts) > 0 {
					...
					cmdShow := exec.Command("bd", "--sandbox", "show", molID)
...
func autoSpawnPatrol(cfg PatrolConfig) (string, error) {
	...
	cmdCatalog := exec.Command("bd", "--sandbox", "mol", "catalog")
	...
	cmdSpawn := exec.Command("bd", "--sandbox", "mol", "wisp", "create", protoID, "--actor", cfg.RoleName)
	...
	cmdPin := exec.Command("bd", "--sandbox", "update", patrolID, "--status=hooked", "--assignee="+cfg.Assignee)
