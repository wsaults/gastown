...
func (b *Beads) run(args ...string) ([]byte, error) {
	// Use --sandbox for faster read operations (avoids daemon IPC overhead)
	...
	fullArgs := append([]string{"--sandbox", "--allow-stale"}, args...)
	...
	// Handle bd --sandbox exit code 0 bug: when issue not found,
	// --sandbox exits 0 but writes error to stderr with empty stdout.
